...
import type { Flat, FlatMarker } from "../types";

interface MapProps {}

export default function MapScreen(_: MapProps) {
  ...
  const handleMarkerPress = (flat: Flat) => {
    if (!selectedFlat) setSelectedFlat(flat);
    else setSelectedFlat(null);
  };

  const handleApply = (flat: Flat) => {
    Linking.openURL(flat.url).catch(error => {
  ...
  const filteredFlats = flats.filter((flat): flat is FlatMarker => flat.longitude !== null && flat.latitude !== null);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.buttonLoadMore} onPress={loadMore}>
        <Text style={styles.loadMoreButtonText}>Load more flats on map...</Text>
      </TouchableOpacity>
        ...
        <View style={styles.preferredFlat}>
          ...
          {!selectedFlat.applied ? (
            <TouchableOpacity style={styles.loadMoreButton} onPress={() => handleApply(selectedFlat)}>
              <Text style={styles.applyButtonText}>Apply to flat</Text>
            </TouchableOpacity>
          ) : (
